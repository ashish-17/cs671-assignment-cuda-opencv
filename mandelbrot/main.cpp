...
int main(int argc, char** argv) {
    ...
    int* output_gpu    = new int[width*height];
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        double endTime = CycleTimer::currentSeconds();
        minGpu = std::min(minGpu, endTime - startTime);
    }

    printf("[mandelbrot gpu]:\t\t[%.3f] ms\n", minGpu * 1000);
    writePPMImage(output_thread, width, height, "mandelbrot-gpu.ppm", maxIterations);
    ...
    if (0 && ! verifyResult (output_serial, output_gpu, width, height)) {
        printf ("Error : Output from gpu does not match serial output\n");
